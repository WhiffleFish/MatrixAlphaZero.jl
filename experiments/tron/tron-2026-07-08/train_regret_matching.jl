...
const EXPERIMENT_NAME = "tron-2026-07-08"
...
function prefixed_az_eval_metrics(result::NamedTuple, prefix::AbstractString, az_player::Int)
    ...
    az_win = az_player == 1 ? result.p1_win_rate : result.p2_win_rate
    opp_win = az_player == 1 ? result.p2_win_rate : result.p1_win_rate

    add_scalar_metric!(pairs, prefix, :reward, reward[az_player])
    add_scalar_metric!(pairs, prefix, :mean_steps, result.mean_steps)
    add_scalar_metric!(pairs, prefix, :az_win_rate, az_win)
    add_scalar_metric!(pairs, prefix, :opp_win_rate, opp_win)
    add_scalar_metric!(pairs, prefix, :draw_rate, result.draw_rate)
...
function print_eval_summary(iter, az_p1_result, az_p2_result)
    ...
    println(
        ...
        "win=$(round(az_p1_result.p1_win_rate; digits=3)) ",
        "draw=$(round(az_p1_result.draw_rate; digits=3)) ",
        "steps=$(round(az_p1_result.mean_steps; digits=1)); ",
        "AZ p2 reward=$(round(az_p2_reward; digits=3)) ",
        "win=$(round(az_p2_result.p2_win_rate; digits=3)) ",
        "draw=$(round(az_p2_result.draw_rate; digits=3)) ",
...
function (cb::StatRolloutEvalCallback)(info::NamedTuple)
    ...
    # both players are symmetric light-cycles, so the same heuristic serves either seat
    az_p1 = Tools.JointPolicy(
        Tools.SinglePlayerAlphaZeroPolicy(planner, 1),
        TronTools.tron_heuristic(cb.game, 2),
    )
    az_p2 = Tools.JointPolicy(
        TronTools.tron_heuristic(cb.game, 1),
