...
function (cb::StatRolloutEvalCallback)(info::NamedTuple)
    ...
    # Keep guaranteed exploration inside the tree search. The deployed action
    # is sampled directly from the returned unperturbed average strategy, so no
    # additional action-selection epsilon is applied by the policy wrapper.
    transfer_search = build_search(
        info.oracle;
        epsilon=_ -> EVAL_SEARCH_EPSILON,
...
wandb_config = Dict{String,Any}(
    ...
    "eval/search_epsilon" => EVAL_SEARCH_EPSILON,
    "eval/action_epsilon" => 0.0,
    "eval_runs" => eval_runs,
