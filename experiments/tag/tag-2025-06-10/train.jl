...
trunk = Chain(Dense(4, 16, tanh), Dense(16, 16, tanh))
...
actor = MultiActor(
    Chain(Dense(16, 16, tanh), Dense(16, na1)), 
    Chain(Dense(16, 16, tanh), Dense(16, na2))
)
oracle = ActorCritic(trunk, actor, critic)
jldsave(joinpath(@__DIR__, "oracle.jld2"); oracle)

sol = MatrixAlphaZero.AlphaZeroSolver(
    oracle=oracle, steps_per_iter=steps_per_iter, max_iter=iter, train_intensity=10,
    buff_cap = 200_000,
