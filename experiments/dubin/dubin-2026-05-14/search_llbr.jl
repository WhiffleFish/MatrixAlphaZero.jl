...
llbr_helpers = joinpath(dirname(dirname(dirname(@__DIR__))), "ExperimentTools", "src", "llbr.jl")
...
@everywhere include($llbr_helpers)
...
@everywhere function run_style_trial(
    ...
    result = search_eval(planner, params, game, s0; ϵ, every, progress = false)
    ...
    try
        for i in 1:tree_queries
            ...
            if iszero(mod(i, every))
                ...
                brv1, brv2 = approx_br_values_both_st(game, planner.oracle, π1, π2, s0; max_depth = 5)
