...
const EXPERIMENT_NAME = "dubin-2026-07-14"
...
const SEARCH_NAME = Dict(
    :value_only => "rm_plus_value_only",
    :transfer_default => "rm_plus_transfer_default",
    :transfer_conservative => "rm_plus_transfer_conservative",
)[CONDITION]
...
function init_oracle(condition, width, na1, na2; value_weight, regret_weight, strategy_weight)
    # Build the shared value path first so every condition starts from the same
    # trunk and critic weights under the common seed.
    trunk = Chain(Dense(8 => width, tanh), Dense(width => width, tanh))
    critic = Chain(Dense(width => width, tanh), Dense(width => 1))
    condition == :value_only && return AZ.CriticOnly(trunk, critic; value_weight)
