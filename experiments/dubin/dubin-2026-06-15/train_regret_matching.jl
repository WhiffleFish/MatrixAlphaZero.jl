...
function prefixed_az_eval_metrics(result::NamedTuple, prefix::AbstractString, az_player::Int; search_epsilon)
    ...
    add_scalar_metric!(pairs, prefix, :search_epsilon, search_epsilon)
...
function (cb::StatRolloutEvalCallback)(info::NamedTuple)
    ...
    metrics = merge(
        prefixed_az_eval_metrics(az_p1_result, "eval/az_p1_vs_heuristic", 1; search_epsilon = eval_search_epsilon(info)),
        prefixed_az_eval_metrics(az_p2_result, "eval/heuristic_vs_az_p2", 2; search_epsilon = eval_search_epsilon(info)),
    )
