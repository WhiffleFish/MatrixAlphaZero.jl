...
const EXPERIMENT_NAME = "dubin-2026-07-15"
const EXPERIMENT_GROUP = "$(EXPERIMENT_NAME)-independent-rm-plus-transfer-500"
const SEED = 0

@isdefined(CONDITION) || error("Define CONDITION before including experiment.jl")
CONDITION in (:value_only, :transfer_default, :transfer_conservative) ||
    error("Unsupported CONDITION=$(CONDITION)")

const SEARCH_NAME = Dict(
    :value_only => "independent_rm_plus_value_only",
    :transfer_default => "independent_rm_plus_transfer_default",
    :transfer_conservative => "independent_rm_plus_transfer_conservative",
)[CONDITION]

const LOSS_SCALED_TRANSFER = if CONDITION == :value_only
    nothing
elseif CONDITION == :transfer_default
    AZ.LossScaledTransfer()
else
    AZ.LossScaledTransfer(regret_scale=0.0625, strategy_scale=0.25)
end
...
function init_oracle(condition, width, na1, na2; value_weight, regret_weight, strategy_weight)
    # Initialize the complete value network first so its weights are identical
    # across conditions. No learned parameters are shared between objectives.
    critic = state_network(width, 1)
    condition == :value_only && return AZ.CriticOnly(critic; value_weight)
...
function build_search(oracle; epsilon, info=nothing)
    ...
    isnothing(LOSS_SCALED_TRANSFER) && return AZ.MCTSSearch(; common...)
...
if oracle isa AZ.FittedRegretModel
    merge!(wandb_config, Dict(
